...
class KityPlayer:
    ...
    def play_index(self, index, resume=False, seek_to=None):
        ...
        if index < 0 or index >= len(self.display_playlist):
            index = 0  # Default to first song if invalid
        ...
        try:
            ...
            err_msg = str(e)
            if len(err_msg) > 45: err_msg = err_msg[:45] + "..." # Truncate long errors
    ...
    def stop(self, reset_seek=True):
        ...
        try:
            if self.playback and self.playback.active:
                self.playback.stop()
                time.sleep(0.05)  # Give the audio thread a moment to release the device
        except:
            pass
    ...
    def toggle_pause(self):
        ...
        if self.is_playing:
            ...
            if self.current_index == -1 and self.display_playlist:
                self.play_index(0)
            else:
                try:
    ...
    while True:
        try:
            ...
            if player.is_playing and player.playback:
                try:
                    # Only check if at least 1 second has passed to avoid false positives during load
                    ...
                    if elapsed > 1.0 and not player.playback.active:
                        ...
                        # If we don't know the duration, assume it finished successfully
                        # If we stopped well before the end of the track, it crashed
                        # (playback position can't be read once inactive, so use wall-clock time)
                        if track_dur > 0 and elapsed < track_dur - 2.0:
                            player.add_log(f"Crashed at {int(elapsed)}s")
                            player.stop()
                        else:
                            # Successfully finished, go to next track
