...
class KityPlayer:
        ...
        try:
            ...
            for p in plugins_dir.iterdir():
                if p.suffix == ".py" or (p.is_dir() and (p / "__init__.py").exists()):
                    name = p.stem
                    try:
                        spec = importlib.util.spec_from_file_location(name, str(p) if p.suffix == ".py" else str(p / "__init__.py"))
                        if spec and spec.loader:
                            mod = importlib.util.module_from_spec(spec)
                            spec.loader.exec_module(mod)
                            if hasattr(mod, "setup"):
                                mod.setup(self)
                                self.add_log(f"Loaded plugin: {name}")
                    except Exception as e:
                        self.add_log(f"Plugin err {name}: {str(e)[:20]}")
        except Exception as e:
            self.add_log(f"Plugins dir err: {str(e)[:20]}")
    ...
    def play_index(self, index, resume=False, seek_to=None):
        ...
        for hook in self.plugin_hooks.get("on_play_request", []):
            try:
                if hook(item):
        ...
        try:
            ...
            for hook in self.plugin_hooks.get("on_play", []):
                try: hook(str(filepath))
                except: pass
    ...
    def stop(self, reset_seek=True):
        ...
        if reset_seek:
            self.elapsed_at_pause = 0
            self.current_filepath = None
            for hook in self.plugin_hooks.get("on_stop", []):
                try: hook()
                except: pass
            if HAS_AUDIO_ANALYSIS:
                with self.sf_lock:
    ...
    def toggle_pause(self):
        ...
        if self.is_playing:
            ...
            if (self.current_index < 0 or
                    ...
                    self.display_playlist[self.current_index][0] == 'folder'):
                for i, item in enumerate(self.display_playlist):
                    if item[0] != 'folder':
                        self.current_index = i
                        break
    ...
    def draw(self):
        ...
        for i, item in enumerate(visible_items):
            ...
            if item[0] == 'folder':
                ...
                is_playing_song = (self.current_filepath == item[2]) if hasattr(self, 'current_filepath') else False
                if is_sel:
                    icon = '>>'
                    ...
                    icon = ' -'
                clean_name = str(item[1]).rsplit('.', 1)[0]
    ...
    while True:
        try:
            ...
            if isinstance(event, MouseEvent):
                ...
                handled_key = False
                for hook in player.plugin_hooks.get("on_key", []):
                    try: 
                        if hook(key_str, action): handled_key = True
                    except: pass
                if handled_key:
                    continue
                    ...
                    if key_str in ["enter"]:
                        if player.input_mode == 'search':
                            ...
                            if raw_text.startswith(":"):
                                cmd = raw_text.lower()
                                handled = False
                                for hook in player.plugin_hooks.get("on_command", []):
                                    try: 
                                        if hook(cmd, raw_text): handled = True
                                    except: pass
                                if handled:
